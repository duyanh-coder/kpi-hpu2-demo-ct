...
function Form({ initial, onSubmit }: { initial?: ScheduledReport | null; onSubmit: (d: any) => void }) {
  ...
  return (
    <form onSubmit={e => { e.preventDefault(); onSubmit(f); }} className="space-y-4">
      ...
      <div><label className="block text-sm font-medium mb-1">Người nhận (email, cách nhau bằng dấu phẩy) *</label><input value={f.recipients} onChange={e => setF({ ...f, recipients: e.target.value })} className="w-full px-3 py-2 border rounded-lg text-sm" placeholder="user1@hpu2.edu.vn, user2@hpu2.edu.vn" required /></div>
      <div className="grid grid-cols-2 gap-4">
